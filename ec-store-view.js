...
export default function ECStoreView({itemState, appPrefs, onListLimitChange,
	onSearchChange, onSearchClick, onPaginationClick, onOrderBy,
	closeModal, onOption, inputChange, session}) {

	let columns = [];
	if (itemState.prefLabels != null && itemState.prefLabels.ADMIN_EC_STORE_PAGE != null) {
		columns = itemState.prefLabels.ADMIN_EC_STORE_PAGE;
	}
	let group = "TABLE1";
  
	let header = "";
	if (itemState.prefTexts.ADMIN_EC_STORE_PAGE != null && itemState.prefTexts.ADMIN_EC_STORE_PAGE.ADMIN_EC_STORE_PAGE_HEADER != null) {
		header = itemState.prefTexts.ADMIN_EC_STORE_PAGE.ADMIN_EC_STORE_PAGE_HEADER.value;
	}
